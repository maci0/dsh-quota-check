...
async function requestToken(url, headers, body) {
    try {
 ...
 */
async function claudeRequests(home, force) {
 ...
 * @returns the usage request, or `[]` when no credential is usable.
 */
async function codexRequests(home, force) {
    ...
    if (refreshToken !== undefined
        ...
        if (rotatedAccess !== undefined) {
            const nextTokens = { ...tokens, access_token: rotatedAccess };
            const rotatedRefresh = stringOf(rotated.body?.['refresh_token']);
            if (rotatedRefresh !== undefined)
                nextTokens['refresh_token'] = rotatedRefresh;
            const rotatedId = stringOf(rotated.body?.['id_token']);
            if (rotatedId !== undefined)
                nextTokens['id_token'] = rotatedId;
...
function newestGrokEntry(store) {
    let best;
    let bestExpiry = '';
 ...
 */
async function grokRequests(home, force) {
    ...
    if (clientId !== undefined && refreshToken !== undefined
        ...
        const rotatedAccess = stringOf(rotated.body?.['access_token']);
        if (rotatedAccess !== undefined) {
            const next = { ...entry, key: rotatedAccess };
            const rotatedRefresh = stringOf(rotated.body?.['refresh_token']);
            if (rotatedRefresh !== undefined)
                next['refresh_token'] = rotatedRefresh;
            const expiresIn = numberOf(rotated.body?.['expires_in']);
            if (expiresIn !== undefined) {
            ...
            await writeJson(path, latest).catch(() => { });
            entry = next;
            token = rotatedAccess;
        }
    }
...
async function cursorFromStateDb(path) {
    try {
        const { DatabaseSync } = await import('node:sqlite');
        const database = new DatabaseSync(path, { readOnly: true, timeout: 1_000 });
        try {
            const rows = database.prepare('SELECT key, value FROM ItemTable WHERE key IN (?, ?)').all('cursorAuth/accessToken', 'cursorAuth/stripeMembershipType');
            const cells = new Map();
            for (const row of rows) {
                const entry = record(row);
                const key = cellString(entry?.['key']);
                if (key !== undefined)
                    cells.set(key, cellString(entry?.['value']));
            }
 ...
 * @returns the usage request, or `[]` when no credential is usable.
 */
async function cursorRequests(home) {
    const fromJson = async (path) => stringOf(record(await readJson(path))?.['accessToken']);
    ...
    if (token === undefined)
        token = (await cursorFromStateDb(cursorStateDb(home)))?.token;
    if (token === undefined)
        return [];
 ...
 */
export async function localRequests(provider, options = {}) {
