 ...
 * selector. Nothing renders while a session has no selection or while the
 * provider publishes no balance or quota route: the statusbar never grows a
 * "no data" row. A lookup that **failed** is the exception — a rejected key or
 * an unreachable endpoint is a broken setup, not absent data, so it renders as
 * a dimmed chip whose tooltip says what went wrong.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.qc-chip[data-level="error"]{color:var(--dsw-alias-label-tertiary)}',
    ...
    function createChip() {
      return function QuotaChip(props) {
        ...
        if (report.status === 'error') {
          // The one failure the statusbar has to admit to: silence here hides a
          // rejected key or an unreachable endpoint behind "nothing to show".
          const failure = [report.displayName, report.message].filter(Boolean).join('\n')
